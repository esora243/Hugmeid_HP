...
import { Filter, Search, MapPin, JapaneseYen, Clock, X, Briefcase } from "lucide-react";
...
export function JobsPageClient({ initialJobs, initialLoadError }: JobsPageClientProps) {
  ...
  const handleSave = async (jobId: string) => {
    ...
    try {
      const saved = await toggleSaved("job", jobId);
        ...
        {loadError ? (
          ...
          filteredJobs.map((job) => (
            <div key={job.id} className="bg-white p-4 rounded-2xl shadow-sm border border-orange-50 relative hover:shadow-md transition-all group">
              <SaveButton compact saved={isSaved("job", job.id)} onClick={() => void handleSave(job.id)} />

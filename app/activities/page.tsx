      ...
      <div className="px-4 pt-3 space-y-4">
        {!hasContent && (
          <div className="bg-white rounded-2xl border border-pink-100 p-8 text-center">
            <Users className="mx-auto text-pink-200 mb-3" size={40} />
        ...
        {activeTab === "groups" && studentGroups.map((group) => (
          <Link key={group.id} href={`/activities/groups/${group.id}`} prefetch={false} className="block bg-white rounded-2xl shadow-sm border border-pink-50 overflow-hidden hover:shadow-md transition-shadow group">
        ...
        {activeTab === "study-abroad" && studyAbroadPrograms.map((program) => {
          const programUrl = normalizeExternalHttpsUrl(program.url);
          return (
            <div key={program.id} className="block bg-white rounded-2xl shadow-sm border border-pink-50 overflow-hidden hover:shadow-md transition-shadow group">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-gray-50">
                  ...
                  {programUrl ? (
                    <a href={programUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs font-bold text-pink-500 hover:text-pink-600">詳細 <ExternalLink size={12} /></a>
        ...
        {activeTab === "articles" && activityArticles.map((article) => {
          const articleUrl = normalizeExternalHttpsUrl(article.url);
          return (
            <a key={article.id} href={articleUrl || "#"} target={articleUrl ? "_blank" : undefined} rel={articleUrl ? "noopener noreferrer" : undefined} className="block bg-white rounded-2xl shadow-sm border border-pink-50 overflow-hidden hover:shadow-md transition-shadow group">
              <div className="flex gap-4 p-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-3 text-xs text-gray-400 flex items-center gap-1"><Newspaper size={12} /> {articleUrl ? "外部記事へ" : "URL未設定"}</div>
                </div>
              </div>
            </a>
          );

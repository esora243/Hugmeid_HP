    ...
    return (
      <div className="min-h-screen bg-white pb-20 flex items-center justify-center">
        <div className="text-center px-6">
          ...
          <button onClick={() => router.push("/activities")} className="bg-orange-500 text-white font-bold px-6 py-3 rounded-full">課外活動へ戻る</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-white pb-20">
      <div className="w-full max-w-lg mx-auto">
        <div className="sticky top-[110px] z-30 bg-white/90 backdrop-blur-md border-b border-orange-100 px-4 py-3 flex items-center gap-3">
          <button onClick={() => router.push("/activities")} className="text-gray-600 hover:text-orange-500"><ArrowLeft size={24} /></button>
        ...
        <div className="animate-fade-in">
          <div className="w-full h-64 bg-gray-100 relative">
            ...
            <div className="absolute top-4 left-4"><span className="bg-orange-500 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-md">{group.category}</span></div>
          </div>
          <div className="px-4 py-6 border-b border-gray-100">
            <h1 className="text-2xl font-bold text-gray-800 mb-3">{group.name}</h1>
            <div className="flex items-center gap-2 text-sm text-gray-600 mb-4"><Users size={16} className="text-orange-400" /><span>{group.members}名のメンバー</span></div>
          ...
          <div className="px-4 py-6 bg-gray-50">
            ...
            <div className="space-y-3">
              {group.social.instagram && (
                <a href={`https://instagram.com/${group.social.instagram.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-4 bg-white rounded-xl border border-gray-100 hover:border-orange-200 hover:bg-orange-50/50 transition-all">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-400 to-orange-400 flex items-center justify-center"><Instagram size={20} className="text-white" /></div>
              ...
              {group.social.twitter && (
                <a href={`https://twitter.com/${group.social.twitter.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-4 bg-white rounded-xl border border-gray-100 hover:border-orange-200 hover:bg-orange-50/50 transition-all">
                  <div className="flex items-center gap-3">
              ...
              {group.social.mail && (
                <a href={`mailto:${group.social.mail}`} className="flex items-center justify-between p-4 bg-white rounded-xl border border-gray-100 hover:border-orange-200 hover:bg-orange-50/50 transition-all">
                  <div className="flex items-center gap-3">

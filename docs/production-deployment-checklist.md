...
This checklist is for the Hugmeid Phase 1 Cloud Run staging and production rollout. It documents the required configuration and verification steps only; it does not authorize an actual deployment by itself.
...
11. Deploy `main` to `hugmeid-web-production` with production database credentials and production LINE/LIFF credentials.

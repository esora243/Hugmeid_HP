...
test("mapTimetableClass maps database relation shape to DTO", () => {
...
test("mapTimetableClass accepts legacy weekday schedules", () => {
  assert.equal(mapTimetableClass(row({ schedule: { weekday: "火", period: 2 } }))?.day, "火");
});

test("mapTimetableClass drops rows without a usable schedule", () => {
  assert.equal(mapTimetableClass(row({ schedule: null })), null);
  assert.equal(mapTimetableClass(row({ schedule: [] })), null);
  assert.equal(mapTimetableClass(row({ schedule: { day: "日", period: 1 } })), null);
  assert.equal(mapTimetableClass(row({ schedule: { day: "月", period: "1" } })), null);
  assert.equal(mapTimetableClass(row({ schedule: { day: "月", period: 0 } })), null);
});
